...
$tmp = $db->query_first("SELECT M.name, M.xmode, M.bbsname, M.innhost, M.innport, M.groups, N.host, N.port FROM _my_dns M, _news_srv N WHERE N.id = M.innsrv AND M.name = '$name' AND M.pass = '$pass' AND M.groups != '' LIMIT 1");
$db->close();
...
if (!is_array($tmp))
		...
		if ($tmp['xmode'] & _INN_PASSIVE_) {
				...
				$xmode = "IHAVE";
				$newsport = $tmp['port'];
				$string = "恭喜您，目前您使用的是被动转信模式。\n";
		}
		else {
				$innmode = "主动式";
				$xmode = "POST";
				$newsport = 119;
				$string = "注意: 目前您使用的是主动转信模式，我们建议您尽快升级到被动模式。\n";
		}
...
cnnews	$newshost	$xmode($newsport)		cn.bbs.*
